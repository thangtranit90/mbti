...
export function VillainsSection({ villains }: Props) {
  ...
  return (
    ...
    >
      <h2
        id="villains-heading"
        className="text-[13px] uppercase tracking-[0.2em] text-slate-400 mb-4"
      >
      ...
      <div className="flex flex-col gap-3">
        {villains.map((v) => (
          <div
            key={v.type}
            className="rounded-2xl border border-[var(--hairline)] bg-surface-elevated shadow-[var(--shadow-e1)] p-4"
          >
            <div className={`text-[15px] font-semibold tracking-wider text-type-${v.type} mb-1`}>
              {v.type}
            </div>
            <p className="text-slate-300 text-[14px] leading-relaxed">{v.reason}</p>
          </div>
        ))}
      </div>
    </motion.section>
